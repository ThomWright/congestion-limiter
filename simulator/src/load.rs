...
impl LoadPattern {
    ...
    pub const fn segments(waypoints: Vec<(Duration, f64)>) -> Self {
    ...
    pub fn next_interarrival(&self, elapsed: Duration, rng: &mut SmallRng) -> Duration {
        match self {
            Self::Constant { dist } => Duration::from_secs_f64(dist.sample(rng)),
            Self::Step { phases } => {
            ...
            Self::Segments { waypoints } => {
                ...
                let mut rps = waypoints.last().map_or(1.0, |&(_, r)| r);
                for &(duration, target_rps) in waypoints {
                    if remaining < duration {
                        let progress = remaining.as_secs_f64() / duration.as_secs_f64();
                        rps = (target_rps - prev_rps).mul_add(progress, prev_rps);
                        break;
                    }
                    remaining -= duration;
                    prev_rps = target_rps;
